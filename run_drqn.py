...
def train(writer, policy_net, target_net, memory, optimizer, batch_size, gamma):
    ...
    actions = torch.cat(actions, dim=1).to(device) # [seq, batch, 1]
    rewards = torch.cat(rewards, dim=1).to(device) # [seq, batch, 1]
    next_observations = torch.cat(next_observations, dim=1).to(device)  # [seq, batch, 4, 64, 64]
    dones = torch.cat(dones, dim=1).to(device) # [seq, batch, 1]
    ...
    for i in range(seq_len):
        ...
        q_target, target_hidden = target_net(next_obs, target_hidden.to(device)) # q_target : (1, 2, 6)
        q_target_max = q_target.max(2)[0].detach().unsqueeze(2) # q_target_max : (1, 2, 1) = (seq, batch, 1)
        targets = rewards[i] + gamma*q_target_max*dones[i] # rewards[i], dones[i] : (batch, 1),

        obs = observations[i]
        hidden = policy_net.init_hidden_state(batch_first=False,
                                              batch_size=batch_size,
                                              training=True)
        q_out, hidden = policy_net(obs, hidden.to(device)) # q_out : (1, 2, 6)
        temp_action = actions[i].clone()  # temp_action : (2, 1)
        temp_action = temp_action.unsqueeze(0)  # temp_action : (1, 2, 1)
        q_a = q_out.gather(2, temp_action) # q_a : (1, 2, 1)
        loss = F.smooth_l1_loss(q_a, targets)
        total_loss += loss

    writer.add_scalar('Rewards per episodes', total_loss)
    optimizer.zero_grad()
...
def run():
    ...
    memory = EpisodeMemory(random_update=random_update,
                           max_epi_num=20, max_epi_len=max_epi_len,
                           batch_size=batch_size,
                           n_step=n_step)
    ...
    for e in range(max_epi):
        state = env.reset()
        obs = converter(env_name, state).to(memory_device) # obs : [1, 4, 64, 64]
        done = False
        ...
        for t in range(max_epi_step):
            action_index, hidden = policy_net.sample_action(obs.to(device="cuda:0"), epsilon, hidden)
            action = make_6action(env, action_index)
            s_prime, reward, done, info = env.step(action)
            obs_prime = converter(env_name, s_prime).to(memory_device)
            done_mask = 0.0 if done else 1.0

            batch_action = torch.tensor([action_index]).unsqueeze(0).to(memory_device)
            batch_reward = torch.tensor([reward]).unsqueeze(0).to(memory_device)
            batch_done = torch.tensor([done_mask]).unsqueeze(0).to(memory_device)
            ...
            if len(memory) > min_epi_num:
                train(writer, policy_net,target_net,memory,optimizer,batch_size, gamma=0.99)
